...
const UpdateStudentContact = asyncHandler(async (req, res) => {
    const {id} = req.params;
    const {
        email, mobilePhone, alternativePhone, currentAddress
    } = req.body;
    ...
    if (!email || !alternativePhone || !currentAddress || !mobilePhone || !alternativePhone) {
        return res.status(412).json({
    ...
    const foundStudent = await studentProfileModel.findByIdAndUpdate(id, {
            $set: {
                contactAddress: {
                    email: email,
